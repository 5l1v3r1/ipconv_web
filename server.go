...
func Subnets(line string) (subnets []CIDR, err error) {
        ...
        if (len(lr) > 2) {
                ...
                return nil, formatError{"There's no NMAP ranges support"}
        } else if (len(lr) == 1) {
...
func main() {
        ...
        m.Post("/convert", binding.Bind(Conv{}), func(r Conv) string {
                var buffer bytes.Buffer
                reader := strings.NewReader(r.List)
              
                scanner := bufio.NewScanner(reader)
                for (scanner.Scan()) {
                        subnets, err := Subnets(scanner.Text())
                        if (err != nil) {
                                continue
                        }
                        ...
                        for _, subnet := range subnets {
                                if (subnet.mask == 32) {
                                        ...
                                        buffer.WriteString(fmt.Sprintf("%s/%d\n", intToIP(subnet.ip), (int)(subnet.mask)))
                                }
                        }
                }
